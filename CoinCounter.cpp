...
CoinCounter::CoinCounter(const cv::Mat& image) {
    mPaperSheetCoordinates = getPaperSheetCoordinates(image);
...
void CoinCounter::drawTotalValue() {
    ...
    cv::putText(mPaperSheetRegion, "Total Value: " + std::to_string(totalValue), textLocation, cv::FONT_HERSHEY_TRIPLEX, 3, textColor);
}
...
void CoinCounter::drawFrame() {
    ...
    cv::line(mPaperSheetRegion, cv::Point(0, mPaperSheetRegion.rows - 1), cv::Point(mPaperSheetRegion.cols - 1, mPaperSheetRegion.rows - 1), color, thickness);
    cv::line(mPaperSheetRegion, cv::Point(mPaperSheetRegion.cols - 1, mPaperSheetRegion.rows - 1), cv::Point(mPaperSheetRegion.cols - 1, 0), color, thickness);
...
std::vector<cv::Point2f> CoinCounter::getPaperSheetCoordinates(const cv::Mat& image) const {
    cv::Mat cannyOut;
    ...
    cv::approxPolyDP(maxCountour, points, perimeter * 0.02, true);
...
std::vector<cv::Point2f> CoinCounter::sortCornerCoordinates(const std::vector<cv::Point2f>& points, const std::vector<cv::Point2f>& imageCorners) const {
